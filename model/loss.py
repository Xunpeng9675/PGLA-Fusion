...
class PerceptualLoss(nn.Module):
    def __init__(self, weight_path=None):
        ...
        if weight_path is not None and os.path.exists(weight_path):
            ...
            try:
                ...
                print(f"Warning: Failed to load VGG16 from network: {e}")
                print("Please manually download the weights file to: ./vgg16-397923af.pth")
                raise
    ...
    def forward(self, x, y):
        ...
        if y.size(1) == 1:  # 如果是单通道图像
            y = y.repeat(1, 3, 1, 1)  # 将单通道扩展为3通道

        # 获取当前设备
        device = x.device  # 获取输入张量的设备（GPU 或 CPU）
